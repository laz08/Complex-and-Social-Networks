...
```{r barplot2, echo=FALSE, out.width="250px", out.height="250px", fig.align="center", fig.cap = "English out-degree distribution (loglog scale)"}
...
Using the out-degree sequences for each different language, we have obtained
the following coeffcients for each one of them:
...
```{r echo=FALSE}
kable(coeffs_table, caption="\\label{tab:table2} Summary of the most likely parameters")
```

Before proceeding onto computing the delta AIC and select the best model, we should first check that
our methods have been correctly addressed and implented.
...
kable(coeffs_table_test, caption="\\label{tab:table3}Summary of the most likely parameters for geometric test sequence")
```

And the delta AIC one:

```{r echo=FALSE}
kable(aic_table_test,  caption="\\label{tab:table4}AIC difference of the Geometric test model")
```

As we can see, in this delta AIC table we can see that the preferred and selected method is the 
geometric one. So in this case, the test is passed.
...
kable(coeffs_table_test,  caption="\\label{tab:table5} Summary of the most likely parameters for Zeta test sequence")
```

And for the delta AIC:
```{r echo=FALSE}
kable(aic_table_test,  caption="\\label{tab:table6} AIC difference of the Zeta test model")
```

In this case, we can see that it is always selecting the Right-truncated zeta.

# Delta AIC of our models
Having obtained the parameters and checked our methods, we can now proceed to obtain the -2 log Likelihood 
for each method and compute the AIC for the real cases.
...
```{r echo=FALSE}
kable(aic_table,  caption="\\label{tab:table7}  AIC difference of our different languages' models")
```
...
```{r  warning=FALSE, echo=FALSE, out.width="250px", out.height="250px", fig.align="center"}
...
coeff = coeffs_table[lang_idx, 6]

zeta_plot(seq, coeff, lang)

```

As we can see, the Right truncated  Zeta distribution fits well with the data's observations
for the coefficient of gamma obtained in the case of English.
...
As we can see, the Right truncated  Zeta distribution fits well with the data's observations
for the coefficient of gamma obtained in the case of Chinese as well.
...
As we can see, the Right truncated  Zeta distribution does not fit as well as with the other
2 languages, but still takes into account the tail on the far right.
