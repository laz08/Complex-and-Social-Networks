...
```{r echo=FALSE}
kable(coeffs_table, caption="\\label{tab:table2}TODO")
```
...
kable(coeffs_table_test, caption="\\label{tab:table3}TODO")
```

And the delta AIC one:

```{r echo=FALSE}
kable(aic_table_test,  caption="\\label{tab:table4}TODO")
```
...
kable(coeffs_table_test,  caption="\\label{tab:table5}TODO")
```

And for the delta AIC:
```{r echo=FALSE}
kable(aic_table_test,  caption="\\label{tab:table6}TODO")
```
...
```{r echo=FALSE}
kable(aic_table,  caption="\\label{tab:table7}TODO")
```
...
To verify this model selection, we are now going to check how the real data 
aligns with the distribution itself. 
