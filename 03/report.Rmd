...
Most importantly, **the computation of the closeness for each vertex is way faster on R than our tests on computing it with ``C++`` with large networks.** So, although it seemed strange to us that this complex computation takes only seconds in R (with some exceptions as we will see a little below from here), we started our project by assuming its correctness and a very efficient implementation thanks to using the iGraph own structure for graphs. 
