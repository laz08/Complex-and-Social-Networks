...
Next, we focus on the Basque language to test our implmentation and determine if we need to use any estimation procedures to speed up the computation time. We compute the closness centrality of the original data as well as the p-values for the binomial and switching methods (with T=20 iterations when computing the models). We have combined this with the results of all languages which can be found in table TODO below. 
